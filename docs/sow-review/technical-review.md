...
| **Verdict**               | **Conditional Pass.** The SOW reflects an above-average Azure-native design with the correct headline services (Azure Container Apps, Azure Cosmos DB, Azure Service Bus, Microsoft Entra ID, Azure OpenAI, Azure AI Content Safety, Azure Front Door, Azure Key Vault, Azure App Configuration, Bicep, GitHub Actions). However, several **MUST-FIX** issues exist around (a) AI model naming and access tier, (b) ambiguous "Service A / Service B" notation that hides architectural decisions, (c) missing SKU/tier specificity that drives SLA and cost, (d) the single-region SLO claim vs. composite reality, and (e) multi-tenant Cosmos DB partitioning strategy not pinned down. Recommend addressing the Critical and High items before signature; Medium and Low items can be tracked in the ADR backlog. |
...
**Observations from Microsoft Learn (verified 2026-05-14)**

1. There is **no separate "GPT-5.2 Vision" SKU** in Microsoft Foundry. The current vision-enabled chat models are the **o-series, GPT-5 series, GPT-4.1 series, GPT-4.5, and GPT-4o series** (`https://learn.microsoft.com/azure/foundry/openai/how-to/gpt-with-vision`). Image input is a *capability* exposed by these chat-completion models, not a distinct deployment name.
...
3. `gpt-5.2` is currently **Limited Access**: "Request access: Limited access model application (`https://aka.ms/oai/gpt5access`). If you already have access to a limited access model no request is required." This contradicts SOW assumption A-7 ("generally available in East US 2 by Sprint 3"). GA timing for `gpt-5.2` cannot be assumed.
4. Regional availability for `gpt-5.2` is governed by the Foundry "Models sold directly by Azure" availability table — not a static "East US 2 by Sprint 3" statement. Many of the latest reasoning SKUs land first in **East US 2, Sweden Central, South Central US, Poland Central** as a Global Standard deployment, not a single-region regional deployment.
5. The service is now branded **"Azure OpenAI in Microsoft Foundry Models"** / **"Azure OpenAI in Azure AI Foundry Models"**. Using the legacy "Azure OpenAI Service" name in a contract is acceptable but ages quickly.
...
1. Florence-2 is a Microsoft vision foundation model distributed through the Foundry / Azure AI model catalog. Per the Foundry model deployment overview (`https://learn.microsoft.com/azure/foundry/concepts/foundry-models-overview`), models in the catalog deploy either via:
   ...
   - **Serverless deployment** — token / call-based, hosted by Microsoft, Pay-as-you-go.
...
2. Microsoft's documented pattern for protecting API Management with WAF is **Azure Front Door Premium → APIM → backend** (`https://learn.microsoft.com/azure/web-application-firewall/afds/protect-api-hosted-apim-by-waf`). Front Door Premium is required if a managed-rule-set WAF is needed (Standard does not include managed rule sets).
...
**Observations from Microsoft Learn** (`https://learn.microsoft.com/azure/reliability/reliability-azure-container-apps`)
...
3. **Min replicas ≥ 2**, configured at creation time (cannot be enabled afterwards).
4. KEDA HTTP scaler is supported; not the only option.

The SOW says "2–10 instances" which satisfies (3), and "multi-AZ" which implies (2), but does **not** state the VNet integration, subnet sizing, or the requirement that zone redundancy is set at creation time and cannot be added later.

**Recommended fix**: in §16.1 / §16.2, add VNet integration, subnet CIDR, and "workload-profiles environment with zone redundancy enabled at creation" as architecture commitments. Add a customer dependency in §17.1 if Walmart provides the VNet.
...
## 5. Cross-references to Microsoft Learn (verified 2026-05-14)
