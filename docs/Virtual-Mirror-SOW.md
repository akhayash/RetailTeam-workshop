...
| T-3 | **Integration / System testing** | .NET Aspire harness with Cosmos emulator + Azurite + mocked AI; end-to-end API smoke. **Minimum scope for D-1 acceptance**: Entra ID auth (valid + invalid token), Cosmos CRUD (create/read/update/delete), Blob lifecycle (upload/download/TTL-delete), Service Bus publish/subscribe, AI-failover circuit-breaker trip, health/readiness probe | Microsoft | Continuous |
...
| PR-7 | .NET 8 runtime not in Walmart's core engineering skill set — long-term ownership risk | **HIGH** | Walmart's primary engineering stack is Java / Node.js / Python; .NET is used internally but not on the "golden path" for customer-facing services. Post-hypercare maintenance requires either: (a) Walmart staffing .NET-skilled engineers, (b) Microsoft extending via a managed-services SOW, or (c) a future rewrite to Java/Kotlin (significant cost). Mitigations: (1) Knowledge-transfer sessions (D-14) include .NET 8 / ASP.NET Core deep-dive; (2) Runbooks written for operators without deep .NET expertise; (3) Service designed as a self-contained API with minimal coupling to Walmart's broader platform; (4) Discuss managed-services extension at Sprint 6 checkpoint. | Sprint 6 Product Council checkpoint: Walmart confirms post-hypercare staffing model (internal .NET team, managed-services SOW, or deferred decision). Decision recorded in ADR. |
...
| R-005 | .NET 8 runtime creates talent-gap for Walmart post-handover | **HIGH** | Microsoft PM (tracking) · Walmart Platform Eng (staffing decision) | Service architected as self-contained API; comprehensive runbooks; knowledge-transfer sessions (D-14); managed-services SOW recommended if Walmart cannot staff .NET engineers within 90 days of go-live | Sprint 6 Product Council confirms post-hypercare ownership model; ADR recorded |
...
**Deemed-acceptance rule**: For deliverables requiring Customer review (D-4, D-9, D-13, D-14), Microsoft will issue a formal **delivery notification** via the agreed project channel (email + tracking tool). If the Customer does not provide written acceptance **or** specific written objections within the stated review period (5 business days unless otherwise specified), the deliverable is **deemed accepted**. Written objections must reference specific acceptance criteria that are not met; general dissatisfaction without criteria reference does not constitute a valid objection.
...
| A-6e | Walmart accepts the **.NET 8 / ASP.NET Core** technology choice for the VirtualMirror service. Walmart's core engineering platform is Java / Node.js / Python; .NET is not on the standard "golden path" for new customer-facing services. This engagement assumes Walmart will either (a) staff or upskill ≥ 2 engineers with .NET 8 / Azure Container Apps proficiency for post-hypercare ownership, (b) extend Microsoft's involvement via a managed-services SOW, or (c) accept the service as a Microsoft-maintained "black box API" consumed via OpenAPI contract. Failure to resolve this by Sprint 6 is a change-management trigger (§12). |
...
|:-:|-------------|----------------------------------|---------------------|:-----------------:|-------|
| D-1 | Foundational platform | Cloud-ready service skeleton with Entra ID auth, Cosmos/Blob/Service Bus repositories, Polly resilience pipelines with AI failover, health probes, observability | Integration test suite (T-3) covering **minimum 6 paths** (Entra ID auth happy + reject, Cosmos CRUD, Blob lifecycle, Service Bus pub/sub, AI-failover circuit-breaker trip) passes green in CI; **H7: AI failover < 5 s** in integration test under simulated primary-provider outage (HTTP 503 injected via Polly chaos policy); readiness probe responds < 1 s; ≥ 80% line coverage on WP1/WP2 code (enforced by D-8 coverage gate) | BO-3 | WP1, WP2 |
| D-2 | Photo-based fit assessment (MVP) | `POST /v1/assessments` accepts photo + height, returns 5-point fit per body area with confidence and disclaimer when < 70% confidence, p95 < 5 s end-to-end | All 3 US1 acceptance scenarios + 5 edge cases in [`spec.md`](../specs/001-clothing-fit-assessment/spec.md) §User Scenarios pass as automated integration tests; **H1: ≤ 15% of calibration photos > ±4 cm** vs. manual ground-truth measurements on a **≥ 50-image diverse calibration set** (≥ 3 body types, ≥ 2 genders, height range 150–195 cm); p95 end-to-end latency < 5 s on 10-concurrent requests; demoable to Customer PO with live photo | BO-1, BO-2 | WP3, US1, FR-001..005, SC-001 |
| D-3 | Measurement profile storage | `POST/GET/DELETE /v1/profiles/{shopperRef}` with explicit consent and 24-hour hard-delete SLA | All 3 US2 acceptance scenarios pass as automated tests; DELETE returns 204 and subsequent GET returns 404; Cosmos soft-delete TTL ≤ 24 h verified via integration test with clock-forward; Blob container purge confirmed; consent reference recorded in audit log with `correlationId` traceable in Application Insights | BO-1, BO-2 | WP4, US2, FR-006, FR-014 |
| D-4 | Frontend integration contract | OpenAPI 3.0.3 contract published, Swashbuckle UI live, URI versioning (`/v1/...`), CORS configured, contract tests against `contracts/openapi.yaml` | OpenAPI spec validates via `openapi-spec-validator` in CI; all contract tests green; Walmart Platform Eng integration review signs off within **5 business days** of delivery notification or deliverable is **deemed accepted** per §15.2 | BO-2, BO-3 | WP5, US3, FR-007, FR-008 |
| D-5 | Garment data ingestion | `POST /v1/garments` (single) and `POST /v1/garments:batch` (≤ 100) with per-SKU validation, partial-failure semantics, version history | All 3 US4 acceptance scenarios pass as automated tests; batch endpoint accepts 100 valid items in < 10 s; rejects invalid items with field-level errors (HTTP 207 Multi-Status with per-item disposition); version history returns ≥ 2 versions on update; partial-failure does not roll back valid items | BO-3 | WP6, US4, FR-009 |
| D-6 | Resilience / degradation ladder | L1–L5 graceful degradation: full pipeline → cached profile → size-chart fallback; Polly + circuit breaker + DLQ; Service Bus overflow queue when queue depth > 50 or p95 > 4 s | Chaos tests (Polly fault-injection on primary AI provider returning HTTP 503 for ≥ 60 s) show **≥ 90% request success (H8)** with responses degrading through L1→L5 as designed; 202 Accepted with queue position returned when overflow triggers; DLQ messages replayable via runbook step | BO-1 | WP2.4, WP3.5, FR-012 |
| D-7 | Bicep IaC (dev / staging / prod) | 12 Bicep modules + 3 environment parameter files for ACA, Cosmos, Blob, Key Vault, App Config, Front Door, observability, networking | `az deployment group what-if` clean (zero errors, zero destructive changes on idempotent re-run) against all 3 environments; modules pass PSRule for Azure linting with zero critical findings; private endpoints verified reachable from ACA via connectivity test; modules decomposed ≤ 300 lines each | BO-3 | WP7.1 |
| D-8 | CI/CD pipeline | GitHub Actions: build → test (unit + contract + integration) → SAST + SCA → SBOM (CycloneDX) → container build + Trivy scan + Notation sign → deploy with canary | Pipeline green end-to-end on `main`; coverage gate ≥ 80% line / ≥ 90% critical-path (assessment + profile + garment services); SBOM artifact attached to release; Trivy scan zero critical/high CVEs; signed image promoted to staging on green; canary 5% → 25% → 100% with automated rollback on error-rate > 5% | BO-3 | WP7.2 |
| D-9 | Observability & runbooks | OTel → Azure Monitor; SLO dashboards (latency, error rate, confidence distribution, AI failover, DLQ depth); 3 runbooks (latency degradation, AI failover, DLQ depth) | Dashboards render live data from staging; alert rules fire within 2 min of injected fault; runbooks reviewed by Walmart Platform Eng within **5 business days** of delivery notification or **deemed accepted**; correlation IDs propagate end-to-end (verified by tracing a single request from API gateway to Cosmos write in Application Insights) | BO-3 | WP7.3, FR-010, FR-011 |
| D-10 | Load and chaos validation | NBomber 500-concurrent smoke + 30-minute soak; chaos / fault-injection on AI providers, Cosmos, Service Bus, Blob | **H3/H5: p95 < 5 s at 500 concurrent** sustained for 30 min; KEDA scales to ≥ 8 instances (max 10); zero OOM kills during soak; **H8: ≥ 90% HTTP 2xx during 10-min chaos window** with simultaneous AI + Cosmos fault injection; test report artifact published to repo | BO-1, BO-3 | WP7.4, SC-004 |
| D-11 | Model card & responsible AI | Model card documenting prompt, model version, training context, bias considerations, known limitations, confidence thresholds | Model card contains **minimum sections**: (1) model identity + version, (2) intended use + out-of-scope use, (3) training/prompt context, (4) performance metrics (H1 results), (5) bias/fairness considerations (body type, gender, skin tone), (6) limitations + failure modes, (7) confidence thresholds + user-facing disclaimers; reviewed and signed by ML Eng + Tech Lead; published in repo; minor-detection refusal verified (returns HTTP 451 + reason) | BO-1 | WP7.5, FR-015 |
| D-12 | DR plan & data classification | DR plan documenting RTO < 1 h, RPO < 15 min for tenant config; data classification labels (transient, ephemeral, opt-in, audit) | DR plan contains recovery procedures for each Azure service in the architecture; data classification labels match `Sessions/Product-definition.md` categories; tabletop walkthrough conducted with ≥ 1 Microsoft + ≥ 1 Customer representative; walkthrough minutes recorded; RTO/RPO targets validated against Cosmos continuous backup + ACA multi-replica restart time | BO-3 | WP7.5 |
| D-13 | DPIA input package | Inputs to Walmart's DPIA: data flow, lawful basis, retention, deletion, transfers, sub-processors, breach notification | DPIA package contains all sections required by Walmart Privacy Office's DPIA template (data-flow diagram, Art. 6 lawful basis, Art. 9 special-category check, retention schedule, deletion-cascade evidence per US2, sub-processor list with DPAs, breach-notification runbook ref); Walmart Privacy Office signs the completeness checklist within **10 business days** or raises specific written objections; no Open / High findings remain at GA | BO-1, BO-2, BO-3 | WP8.4 |
| D-14 | Knowledge transfer & hypercare | 2 recorded enablement sessions for Walmart Platform Eng, runbooks signed, hypercare on-call schedule for 2 weeks post-go-live | ≥ 2 sessions delivered (recorded, ≥ 45 min each, covering architecture + operations); attendance by ≥ 2 Walmart Platform Eng confirmed; runbook acknowledgement sign-off received or **deemed accepted** after 5 business days; on-call rota agreed and published; no P1 unresolved at end of hypercare period | BO-3 | WP7.5, WP8 |
...
| 0.3.0 | 2026-05-14 | Microsoft ISD draft | Draft for internal review | Added .NET 8 technology-ownership risk: (1) §13.1 PR-7 — new HIGH plan risk documenting that .NET is not in Walmart's core engineering skill set, with 4 mitigations and Sprint 6 validation checkpoint; (2) §13.2 R-005 — new HIGH technical risk for post-handover talent gap with Owner + Validation columns; (3) §18.1 A-6e — new engagement assumption explicitly acknowledging .NET stack choice vs. Walmart's Java/Node/Python golden path, requiring resolution by Sprint 6 (staff .NET engineers, managed-services SOW, or black-box API model). No contract envelope changes. |
